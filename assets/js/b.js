...
if (pelanggaran >= 3 && !sudahDikirim && !sudahSelesai){
...
document.addEventListener("change", (e) => {
  ...
  if (e.target.type === "checkbox") {
    ...
    if (e.target.checked) {
      if (!jawabanSiswa.mcma[soalId].includes(value)) {
        jawabanSiswa.mcma[soalId].push(value);
      }
    } else {
      jawabanSiswa.mcma[soalId] =
        jawabanSiswa.mcma[soalId].filter(v => v !== value);
    }
  }
  autosaveFirestore();
...
// 🔥 REALTIME PELANGGARAN (GANTI getDoc)
onSnapshot(doc(db, "peserta", siswaUid), (snap) => {
  if (!snap.exists()) return;

  const data = snap.data();
  pelanggaran = data.jumlahPelanggaran || 0;

  console.log("🔥 realtime pelanggaran:", pelanggaran);
});
